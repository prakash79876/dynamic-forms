...
class MySubmissionController extends Controller
{
    ...
    {
        $user = auth()->user();
        $submission = Submission::where(['user_id' => $user->id, 'id' => $id])->with('form')->firstOrFail();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('formbuilder::my-submissions.index')->with('success', 'Submission updated.');
        } catch (Throwable $e) {
            info($e);
            DB::rollback();
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('formbuilder::my-submissions.index')->with('success', 'Submission deleted!');
    }
}
